...
//生产者发布流程
func Publish(key string, body []byte) error {
	// 初始化mq
	// mq, err := newRabbitMQ("callback-queue", "callback-producter", key)
	// if err != nil {
	// 	logger.Error("[MQ] Publish Err ", err)
	// 	return err
	// }
	...
	*/
	_, err := Rmq.Channel.QueueDeclare( // 返回的队列对象内部记录了队列的一些信息，这里没什么用
		Rmq.QueueName, // 队列名
		true,          // 是否持久化
	...
	err = Rmq.Channel.QueueBind(
		Rmq.QueueName,  // 绑定的队列名称
		Rmq.RoutingKey, // bindkey 用于消息路由分发的key
		Rmq.Exchange,   // 绑定的exchange名
		false,          // 是否阻塞
		nil,            // 额外属性
	)
	...
	Rmq.Channel.Publish(
		...
		amqp.Publishing{ // 发送的消息，固定有消息体和一些额外的消息头，包中提供了封装对象
			ContentType: ContentType, // 消息内容的类型
			Body:        body,        // 消息内容
		},
	)
